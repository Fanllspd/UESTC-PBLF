...
DialogAdd::DialogAdd(QWidget *parent, BooksList *BL, QStandardItemModel *model)
    : QDialog(parent), booksList(BL), model(model)
    , ui(new Ui::DialogAdd)
{
    ui->setupUi(this);
    ...
    //this->ui->buttonBox->setDefaultButton(nullptr);
    //connect(this->ui->buttonBox, &QDialogButtonBox::accepted, this, &DialogAdd::on_buttonBox_accepted);
    ...
    //ui->lineEd->setValidator(new QRegExpValidator(QRegExp("\\S+"), this));
...
void DialogAdd::paintEvent(QPaintEvent *event)
{
...
void DialogAdd::on_toolButton_close_clicked()
{
    DialogAdd::close();
}
...
void DialogAdd::mousePressEvent(QMouseEvent *event)
{
...
void DialogAdd::mouseMoveEvent(QMouseEvent *event)
{
    if(event->buttons() == Qt::LeftButton && mousePressed)
    {
        //当窗口最大化或最小化时也不进行触发

        if(DialogAdd::isMaximized() || DialogAdd::isMinimized())
            return;
        else
        {
            ...
            if (!ui->toolButton_close->underMouse())
            {
                ...
                if(event->pos().y()<=0 || event->pos().y()>maxHeight)
                    return;
                DialogAdd::move(event->globalPos() - dragPosition);//移动
            }
...
void DialogAdd::mouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
        mousePressed = false;
}

void DialogAdd::startShakeAnimation(QLineEdit *lineEdit) {
    // 如果已经有动画在运行，不启动新动画
...
bool DialogAdd::isInputNoSpace(const QString &input) {
    return !input.trimmed().isEmpty();
}

bool DialogAdd::areAllLineEditsFilled(const QList<QLineEdit *> &lineEdits)
{
...
void DialogAdd::accept()
{
    ...
    if(areAllLineEditsFilled(lineEdits)) {
        if(InsertBook(booksList, lineEdits, InsertPos,
...
void DialogAdd::reject()
{
    QDialog::reject();
}


void DialogAdd::isLineEditTextChanged(const QString &text, QLineEdit *lineEdit)
{
    if (text.isEmpty()) {
        lineEdit->setStyleSheet("border:1px solid #e74c3c;");
    } else {
...
void DialogAdd::on_lineEdit_InsertPos_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_Title_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_Author_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_Publisher_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_PublishDate_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_Language_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_CopyAmount_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_ISBN_textChanged(const QString &arg1)
{
...
void DialogAdd::on_lineEdit_Keywords_textChanged(const QString &arg1)
{
    isLineEditTextChanged(arg1, this->ui->lineEdit_Keywords);
}


void DialogAdd::on_lineEdit_Category_textChanged(const QString &arg1)
{
...
void DialogAdd::on_SwitchButton_Available_clicked(bool checked)
{
    Available = checked;
}

void DialogAdd::on_SwitchButton_eBook_clicked(bool checked)
{
    eBook = checked;
}
